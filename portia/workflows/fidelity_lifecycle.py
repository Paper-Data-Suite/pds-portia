"""Lifecycle planning for canonical ``fidelity@1`` records."""
...
_FIDELITY_REVISION_MUTABLE_FIELDS = frozenset(
    {"status", "updated_at", "updated_by"}
)


@dataclass(frozen=True, slots=True)
class FidelityLifecycleState:
    """One exact linear lifecycle chain selected for one Fidelity."""
    ...
    def selected_status(self) -> str | None:
        ...
        if not isinstance(value, str):
            raise WorkflowOwnershipError(
                "selected Fidelity lifecycle head has no valid to_status"
            )
        return value
...
def _require_owner(work: ExactPortiaWorkRef, record: PortiaRecord) -> None:
    if (
        ...
        raise WorkflowOwnershipError(
            "Fidelity lifecycle requires exact support_process@1 ownership"
        )


def _fidelity_target(record: PortiaRecord) -> dict[str, object]:
    if record.logical_id is None:
        raise WorkflowOwnershipError(
            "Fidelity lifecycle target requires an exact Fidelity identity"
        )
    return {
        "kind": "local_record",
...
def _previous_transition_id(record: PortiaRecord) -> str | None:
    ...
    if not isinstance(previous, Mapping):
        raise WorkflowOwnershipError(
            "Fidelity lifecycle previous_transition is malformed"
        )
    if (
        ...
        raise WorkflowOwnershipError(
            "Fidelity lifecycle previous_transition is not exact"
        )
    return str(previous["record_id"])


def fidelity_lifecycle_state(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    fidelity: PortiaRecord,
) -> FidelityLifecycleState:
    """Resolve persisted Fidelity lifecycle history without timestamp ordering."""
    _require_owner(work, fidelity)
    selected = tuple(
        stored
        ...
        if stored.record.field("target") == _fidelity_target(fidelity)
    )
    if not selected:
        return FidelityLifecycleState((), None)
    ...
    for stored in selected:
        ...
        if transition_id is None or transition_id in by_id:
            raise WorkflowPrerequisiteError(
                "Fidelity lifecycle history repeats a transition identity"
            )
    ...
    if any(identifier not in by_id for identifier in referenced):
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle history references a missing predecessor"
        )
    if len(roots) != 1:
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle history must contain exactly one root"
        )
    if any(count != 1 for count in referenced.values()):
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle history contains a fork"
        )

    head_ids = [identifier for identifier in by_id if identifier not in referenced]
    if len(head_ids) != 1:
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle history must contain exactly one head"
        )
    ...
    while True:
        ...
        if current_id is None or current_id in visited:
            raise WorkflowPrerequisiteError(
                "Fidelity lifecycle history contains a cycle"
            )
        ...
        if previous.record.field("to_status") != current.record.field("from_status"):
            raise WorkflowPrerequisiteError(
                "Fidelity lifecycle predecessor status does not reconcile"
            )
        current = previous
    if len(visited) != len(selected):
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle history contains a disconnected transition"
        )
    return FidelityLifecycleState(selected, head)


def require_fidelity_lifecycle_reconciled(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    fidelity: PortiaRecord,
) -> FidelityLifecycleState:
    """Require lifecycle head and canonical Fidelity status to agree."""
    state = fidelity_lifecycle_state(repository, work, fidelity)
    selected_status = state.selected_status
    if selected_status is not None and selected_status != fidelity.status:
        raise WorkflowPrerequisiteError(
            "canonical Fidelity status does not reconcile with lifecycle history"
        )
    return state


def require_fidelity_revision_invariants(
    prior: PortiaRecord,
    ...
    require_revision_invariants(
        prior,
        candidate,
        transitions=_FIDELITY_STATUS_TRANSITIONS,
    )
    prior_data = prior.to_dict()
    candidate_data = candidate.to_dict()
    fields = set(prior_data) | set(candidate_data)
    for field in sorted(fields - _FIDELITY_REVISION_MUTABLE_FIELDS):
        if prior_data.get(field) != candidate_data.get(field):
            raise WorkflowPrerequisiteError(
                "ordinary Fidelity lifecycle replacement cannot rewrite "
                f"field {field}"
            )


def require_coordinated_fidelity_transition(
    prior: PortiaRecord,
    candidate: PortiaRecord,
) -> None:
    """Require one legal non-supersession Fidelity lifecycle change."""
    require_fidelity_revision_invariants(prior, candidate)
    if prior.status == candidate.status:
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle coordination requires a status change"
        )
    if candidate.status == "superseded":
        raise WorkflowPrerequisiteError(
            "Fidelity supersession requires the correction workflow"
        )
    if candidate.status not in {"active", "invalidated"}:
        raise WorkflowPrerequisiteError(
            f"ordinary Fidelity lifecycle cannot select {candidate.status!r}"
        )
    ...
    *,
    ...
    if to_status == "active":
        ...
        if not allow_supersession:
            raise WorkflowPrerequisiteError(
                "Fidelity supersession requires correction workflow"
            )
        ...
        raise WorkflowPrerequisiteError(
            f"Fidelity lifecycle cannot select status {to_status!r}"
        )
    if reason_code == "other":
        if not isinstance(reason_detail, str) or not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Fidelity lifecycle reason 'other' requires detail"
            )
    ...
    if reason_detail is not None:
        if not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Fidelity lifecycle reason detail cannot be empty"
            )
        reason["detail"] = reason_detail
    return reason


def build_fidelity_lifecycle_transition(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    prior: PortiaRecord,
    candidate: PortiaRecord,
    *,
    ...
    if allow_supersession:
        ...
        if candidate.status != "superseded":
            raise WorkflowPrerequisiteError(
                "Fidelity successor coordination must supersede its predecessor"
            )
    else:
        require_coordinated_fidelity_transition(prior, candidate)
    state = require_fidelity_lifecycle_reconciled(
        repository,
        work,
        prior,
    )
    ...
    if not isinstance(created_at, str) or not isinstance(created_by, Mapping):
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle transition requires candidate update provenance"
        )
    effective = effective_at or created_at
    created_time = _parsed_timestamp(
        created_at,
        "Fidelity transition created_at",
    )
    effective_time = _parsed_timestamp(
        effective,
        "Fidelity transition effective_at",
    )
    fidelity_created = _parsed_timestamp(
        prior.to_dict().get("created_at"),
        "Fidelity created_at",
    )
    if effective_time < fidelity_created:
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle effective_at cannot precede "
            "Fidelity creation"
        )
    if effective_time > created_time:
        raise WorkflowPrerequisiteError(
            "Fidelity lifecycle effective_at cannot follow transition creation"
        )
    ...
    if state.head is not None:
        ...
        if head_id is None:
            raise WorkflowOwnershipError(
                "selected Fidelity lifecycle head has no identity"
            )
