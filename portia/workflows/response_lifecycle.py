"""Lifecycle planning and reconciliation for Event-local ``response@1`` records."""
...
_RESPONSE_REVISION_MUTABLE_FIELDS = frozenset({"status", "updated_at", "updated_by"})


@dataclass(frozen=True, slots=True)
class ResponseLifecycleState:
    """One exact linear lifecycle chain selected for a Response record."""
    ...
    def selected_status(self) -> str | None:
        ...
        if not isinstance(value, str):
            raise WorkflowOwnershipError(
                "selected Response lifecycle head has no valid to_status"
            )
        return value
...
def _previous_transition_id(record: PortiaRecord) -> str | None:
    ...
    if not isinstance(previous, Mapping):
        raise WorkflowOwnershipError(
            "Response lifecycle previous_transition is malformed"
        )
    if (
        ...
        raise WorkflowOwnershipError(
            "Response lifecycle previous_transition is not an exact "
            "lifecycle_transition@1 reference"
        )
    return str(previous["record_id"])


def _lifecycle_target(response: PortiaRecord) -> dict[str, object]:
    if response.contract != "response" or response.contract_version != "1":
        raise WorkflowOwnershipError(
            "Response lifecycle target requires response@1"
        )
    if response.logical_id is None:
        raise WorkflowOwnershipError(
            "Response lifecycle target requires an exact Response identity"
        )
    return {
        "kind": "local_record",
...
def _targets_response(transition: PortiaRecord, response: PortiaRecord) -> bool:
    return transition.field("target") == _lifecycle_target(response)


def response_lifecycle_state(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    response: PortiaRecord,
) -> ResponseLifecycleState:
    """Resolve one exact linear Response lifecycle chain without timestamp sorting."""
    require_response_record_owner(work, response)
    selected = tuple(
        stored
        ...
        if _targets_response(stored.record, response)
    )
    if not selected:
        return ResponseLifecycleState((), None)
    ...
    for stored in selected:
        ...
        if transition_id is None:
            raise WorkflowOwnershipError(
                "Response lifecycle transition has no exact identity"
            )
        if transition_id in by_id:
            raise WorkflowPrerequisiteError(
                "Response lifecycle history repeats a transition identity"
            )
    ...
    if missing:
        raise WorkflowPrerequisiteError(
            "Response lifecycle history references a missing predecessor"
        )
    if len(roots) != 1:
        raise WorkflowPrerequisiteError(
            "Response lifecycle history must contain exactly one root transition"
        )
    if any(count != 1 for count in referenced.values()):
        raise WorkflowPrerequisiteError(
            "Response lifecycle history contains a fork"
        )
    ...
    if len(head_ids) != 1:
        raise WorkflowPrerequisiteError(
            "Response lifecycle history must contain exactly one selected head"
        )
    ...
    while True:
        ...
        if current_id is None or current_id in visited:
            raise WorkflowPrerequisiteError(
                "Response lifecycle history contains a cycle"
            )
        ...
        if previous.record.field("to_status") != current.record.field("from_status"):
            raise WorkflowPrerequisiteError(
                "Response lifecycle predecessor status does not reconcile "
                "with its successor"
            )
        current = previous
    ...
    if len(visited) != len(selected):
        raise WorkflowPrerequisiteError(
            "Response lifecycle history contains a disconnected transition"
        )
    return ResponseLifecycleState(selected, head)


def require_response_lifecycle_reconciled(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    response: PortiaRecord,
) -> ResponseLifecycleState:
    """Require the selected lifecycle head, when present, to equal canonical status."""
    state = response_lifecycle_state(repository, work, response)
    selected_status = state.selected_status
    if selected_status is not None and selected_status != response.status:
        raise WorkflowPrerequisiteError(
            "canonical Response status does not reconcile with lifecycle history head"
        )
    return state
...
def require_response_revision_invariants(
    prior: PortiaRecord,
    candidate: PortiaRecord,
) -> None:
    """Allow only lifecycle metadata to change in an ordinary Response transition."""
    if prior.contract != "response" or candidate.contract != "response":
        raise WorkflowOwnershipError(
            "Response revision invariants require response@1 records"
        )
    require_revision_invariants(
        prior,
        candidate,
        transitions=_RESPONSE_STATUS_TRANSITIONS,
    )
    prior_data = prior.to_dict()
    candidate_data = candidate.to_dict()
    comparable_fields = set(prior_data) | set(candidate_data)
    for field in sorted(comparable_fields - _RESPONSE_REVISION_MUTABLE_FIELDS):
        if prior_data.get(field) != candidate_data.get(field):
            raise WorkflowPrerequisiteError(
                f"ordinary Response lifecycle replacement cannot rewrite field {field}"
            )


def require_coordinated_response_transition(
    prior: PortiaRecord,
    candidate: PortiaRecord,
) -> None:
    """Require one legal non-supersession Response status change."""
    require_response_revision_invariants(prior, candidate)
    if prior.status == candidate.status:
        raise WorkflowPrerequisiteError(
            "Response lifecycle coordination requires a status change"
        )
    if candidate.status == "superseded":
        raise WorkflowPrerequisiteError(
            "Response supersession requires the material successor/"
            "correction workflow"
        )
    if candidate.status not in {"active", "invalidated"}:
        raise WorkflowPrerequisiteError(
            f"ordinary Response lifecycle cannot select status {candidate.status!r}"
        )
...
def _transition_reason(
    ...
    *,
    ...
    if to_status == "active":
        ...
        raise WorkflowPrerequisiteError(
            f"Response lifecycle cannot select status {to_status!r}"
        )
...
def build_response_lifecycle_transition(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    prior: PortiaRecord,
    candidate: PortiaRecord,
    *,
    ...
    if allow_supersession:
        ...
        if candidate.status != "superseded":
            raise WorkflowPrerequisiteError(
                "Response successor coordination must supersede its predecessor"
            )
    else:
        require_coordinated_response_transition(prior, candidate)
    state = require_response_lifecycle_reconciled(repository, work, prior)
    if not isinstance(prior.status, str) or not isinstance(candidate.status, str):
        raise WorkflowPrerequisiteError("Response lifecycle status is incomplete")
    ...
    if not isinstance(created_at, str) or not isinstance(created_by, Mapping):
        raise WorkflowPrerequisiteError(
            "Response lifecycle transition requires candidate update provenance"
        )
    effective = effective_at or created_at
    created = _parsed_timestamp(created_at, "Response lifecycle created_at")
    effective_time = _parsed_timestamp(effective, "Response lifecycle effective_at")
    response_created = _parsed_timestamp(
        prior.to_dict().get("created_at"), "Response created_at"
    )
    if effective_time < response_created:
        raise WorkflowPrerequisiteError(
            "Response lifecycle effective_at cannot precede Response creation"
        )
    if effective_time > created:
        raise WorkflowPrerequisiteError(
            "Response lifecycle effective_at cannot follow transition creation"
        )
    ...
    if state.head is not None:
        ...
        if head_id is None:
            raise WorkflowOwnershipError(
                "selected Response lifecycle head has no identity"
            )
