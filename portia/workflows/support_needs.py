"""Production workflow for canonical ``support_need@1`` planning records."""
...
from portia.models import PortiaRecord, SupportNeedV1
...
from portia.workflows.support_need_lifecycle import (
    build_support_need_lifecycle_transition,
    require_coordinated_support_need_transition,
    require_support_need_lifecycle_reconciled,
)
from portia.workflows.support_need_supersession import (
    require_exact_support_need_correction_predecessor,
    require_material_support_need_correction,
    require_support_need_supersession_effective,
    superseded_support_need_predecessor,
    support_need_supersession_ancestry,
    support_need_supersession_reason_detail,
)
...
SUPPORT_NEED_VERSION = "1"
SUPPORT_PROCESS_VERSION = "1"
_PARTICIPANT_VERSION = "1"
_AUTHORING_WORKFLOW_STATES = frozenset({"planning", "active", "paused"})


def support_need_reference(
    work: ExactPortiaWorkRef,
    need_id: str,
    *,
    version: str = SUPPORT_NEED_VERSION,
) -> ExactPortiaWorkRecordRef:
    """Construct one exact Support Process-local Support Need reference."""
...
def _require_support_process_owner(work: ExactPortiaWorkRef) -> None:
    if (
        ...
        raise WorkflowOwnershipError(
            "Support Need workflows require exact support_process@1 ownership"
        )
...
def _parse_timestamp(value: object, *, field_name: str) -> datetime:
    if not isinstance(value, str):
        raise WorkflowPrerequisiteError(
            f"Support Need {field_name} timestamp is malformed"
        )
    ...
    if parsed.utcoffset() is None:
        raise WorkflowPrerequisiteError(
            f"Support Need {field_name} timestamp lacks an explicit offset"
        )
    return parsed
...
def _participant_local_reference(value: object) -> tuple[str, str]:
    if not isinstance(value, Mapping):
        raise WorkflowOwnershipError("Support Need participant target is malformed")
    ...
    if record_ref.get("record_kind") != "support_process_participant":
        raise WorkflowOwnershipError(
            "Support Need target must name a Support Process Participant"
        )
    ...
    if not isinstance(identifier, str) or version != _PARTICIPANT_VERSION:
        raise WorkflowOwnershipError(
            "Support Need target must name an exact support_process_participant@1"
        )
...
def _target_participant_keys(record: PortiaRecord) -> tuple[tuple[str, str], ...]:
    ...
    if not isinstance(target, Mapping):
        raise WorkflowOwnershipError("Support Need target is malformed")
    ...
    if kind != "support_process_participants":
        raise WorkflowOwnershipError("Support Need target kind is unsupported")
    targets = target.get("targets")
    if not isinstance(targets, Sequence) or isinstance(targets, (str, bytes)):
        raise WorkflowOwnershipError("Support Need participant target set is malformed")
    keys = tuple(_participant_local_reference(value) for value in targets)
    if len(set(keys)) != len(keys):
        raise WorkflowPrerequisiteError(
            "Support Need target set repeats a logical participant"
        )
    return keys
...
def _participant_logical_person_identity(
    ...
    if not isinstance(person, Mapping):
        raise WorkflowOwnershipError(
            "Support Need target Participant person is malformed"
        )
    ...
    if kind == "actor":
        ...
        if not isinstance(actor_id, str):
            raise WorkflowOwnershipError(
                "Support Need target Actor reference is malformed"
            )
    ...
    if kind in {"descriptive_person", "unidentified_person"}:
        return None
    raise WorkflowOwnershipError(
        "Support Need target Participant person kind is unsupported"
    )
...
class SupportNeedWorkflowService(WorkflowServiceBase):
    ...
    def _require_authoring_owner(self, work: ExactPortiaWorkRef) -> StoredRecord:
        ...
        if owner.record.status not in {"proposed", "active"}:
            raise WorkflowPrerequisiteError(
                "Support Need authoring requires proposed or active Support Process"
            )
        workflow_state = owner.record.field("workflow_state")
        if workflow_state not in _AUTHORING_WORKFLOW_STATES:
            raise WorkflowPrerequisiteError(
                "Support Need authoring requires planning, active, or paused "
                "Support Process workflow state"
            )
        return owner

    @staticmethod
    def _require_fresh_digital_candidate(candidate: SupportNeedV1) -> None:
        if candidate.status not in {"proposed", "active"}:
            raise WorkflowPrerequisiteError(
                "new Support Need must begin proposed or active"
            )
        ...
        if source_type != "digital_entry":
            raise WorkflowPrerequisiteError(
                "v0.2 Support Need authoring supports digital_entry only"
            )
        if candidate.field("supersedes") is not None:
            raise WorkflowPrerequisiteError(
                "fresh Support Need creation cannot establish supersession history"
            )
        ...
        if updated < created:
            raise WorkflowPrerequisiteError(
                "Support Need updated_at cannot precede created_at"
            )
    ...
    def _require_write_input(
        ...
        if not isinstance(record, SupportNeedV1):
            raise WorkflowOwnershipError(
                "Support Need writes require support_need@1 input"
            )
        if record.class_id != work.class_id or record.work_id != work.work_id:
            raise WorkflowOwnershipError(
                "Support Need does not belong to selected Support Process"
            )
        return record
    ...
    def _require_existing_revision_input(
        ...
        if source_type != "digital_entry":
            raise WorkflowPrerequisiteError(
                "v0.2 Support Need revision/current use requires digital_entry "
                "materialization"
            )
        created = _parse_timestamp(
            candidate.field("created_at"), field_name="created_at"
        ...
        *,
        ...
        for participant_id, version in _target_participant_keys(record):
            ...
            try:
                ...
                raise WorkflowPrerequisiteError(
                    "Support Need target participant does not resolve in owning "
                    "Support Process"
                ) from exc
    ...
    def _graph_records(
        ...
        predecessors = support_need_supersession_ancestry(
            self.repository,
            work,
            candidate,
        )
    ...
    def load_exact(self, reference: ExactPortiaWorkRecordRef) -> StoredRecord:
        ...
        if (
            ...
            or reference.record_ref.contract_version != SUPPORT_NEED_VERSION
        ):
            raise WorkflowOwnershipError(
                "Support Need exact read requires support_need@1 reference"
            )
        return self.repository.load_work_record(
            work,
            "support_need",
            SUPPORT_NEED_VERSION,
    ...
    def list(self, work: ExactPortiaWorkRef) -> tuple[StoredRecord, ...]:
        ...
        return self.repository.list_work_records(
            work,
            "support_need",
            version=SUPPORT_NEED_VERSION,
        )

    list_support_needs = list
    ...
    def transition_lifecycle(
        ...
        *,
        ...
        """Persist one ordinary Support Need activation/invalidation."""
        if (
            reference.record_ref.record_kind != "support_need"
            or reference.record_ref.contract_version != SUPPORT_NEED_VERSION
        ):
            raise WorkflowOwnershipError(
                "Support Need lifecycle requires exact support_need@1 reference"
            )
        work = reference.work_ref
        ...
        def validate_transition(
            ...
            require_coordinated_support_need_transition(prior, revision)
            self.quarantine.require_allowed(work_target(work), "block_work_writes")
            self.quarantine.require_allowed(
    ...
    def _require_successor_candidate(
        ...
        *,
        ...
        require_support_need_lifecycle_reconciled(self.repository, work, prior)
        ...
        if successor_updated < prior_updated:
            raise WorkflowPrerequisiteError(
                "Support Need successor updated_at cannot precede predecessor update"
            )
        require_material_support_need_correction(
            prior,
            value,
    ...
    def correct(
        ...
        *,
        ...
        """Create a corrected Need successor and supersede its predecessor."""
        if (
            predecessor.record_ref.record_kind != "support_need"
            or predecessor.record_ref.contract_version != SUPPORT_NEED_VERSION
        ):
            raise WorkflowOwnershipError(
                "Support Need correction requires exact support_need@1 predecessor"
            )
        work = predecessor.work_ref
        supersession_reason = require_exact_support_need_correction_predecessor(
            work,
            predecessor,
            successor,
        )
        reason_detail = support_need_supersession_reason_detail(successor)
        coordinator = ActionLifecycleCoordinator(
            self.workspace_root,
            repository=self.repository,
            quarantine=self.quarantine,
        ...
        result = coordinator.commit_correction(
            ...
            predecessor_factory=superseded_support_need_predecessor,
            transition_factory=lambda prior, value: (
                build_support_need_lifecycle_transition(
                    self.repository,
                    work,
    ...
    def require_current_use(
        ...
        need = self.load_exact(reference)
        require_support_need_lifecycle_reconciled(
            self.repository, reference.work_ref, need.record
        )
        if need.record.status != "active":
            raise WorkflowPrerequisiteError(
                "current Support Need use requires active canonical status"
            )
        predecessors = support_need_supersession_ancestry(
            self.repository,
            reference.work_ref,
        ...
        require_support_need_supersession_effective(predecessors)
        ...
        source = need.record.field("creation_source")
        source_type = source.get("type") if isinstance(source, Mapping) else None
        if source_type != "digital_entry":
            raise WorkflowPrerequisiteError(
                "Support Need current use currently requires digital_entry "
        ...
        self.quarantine.require_allowed(
            record_target(reference.work_ref, need.record), "block_current_use"
        )
        validate_support_process_graph(
            self.contexts,
            self._graph_records(reference.work_ref, owner, need.record),
        )
        return need
