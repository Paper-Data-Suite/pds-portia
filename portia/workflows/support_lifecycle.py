"""Lifecycle planning for canonical ``support@1`` records."""
...
_SUPPORT_REVISION_MUTABLE_FIELDS = frozenset({"status", "updated_at", "updated_by"})


@dataclass(frozen=True, slots=True)
class SupportLifecycleState:
    """One exact linear lifecycle chain selected for one Support."""
    ...
    def selected_status(self) -> str | None:
        ...
        if not isinstance(value, str):
            raise WorkflowOwnershipError(
                "selected Support lifecycle head has no valid to_status"
            )
        return value
...
def _support_target(record: PortiaRecord) -> dict[str, object]:
    if record.logical_id is None:
        raise WorkflowOwnershipError(
            "Support lifecycle target requires an exact Support identity"
        )
    return {
        "kind": "local_record",
...
def _previous_transition_id(record: PortiaRecord) -> str | None:
    ...
    if not isinstance(previous, Mapping):
        raise WorkflowOwnershipError(
            "Support lifecycle previous_transition is malformed"
        )
    if (
        ...
        raise WorkflowOwnershipError(
            "Support lifecycle previous_transition is not exact"
        )
    return str(previous["record_id"])


def support_lifecycle_state(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    support: PortiaRecord,
) -> SupportLifecycleState:
    """Resolve persisted Support lifecycle history without timestamp ordering."""
    _require_owner(work, support)
    selected = tuple(
        stored
        ...
        if stored.record.field("target") == _support_target(support)
    )
    if not selected:
        return SupportLifecycleState((), None)
    ...
    for stored in selected:
        ...
        if transition_id is None or transition_id in by_id:
            raise WorkflowPrerequisiteError(
                "Support lifecycle history repeats a transition identity"
            )
    ...
    if any(identifier not in by_id for identifier in referenced):
        raise WorkflowPrerequisiteError(
            "Support lifecycle history references a missing predecessor"
        )
    if len(roots) != 1:
        raise WorkflowPrerequisiteError(
            "Support lifecycle history must contain exactly one root"
        )
    if any(count != 1 for count in referenced.values()):
        raise WorkflowPrerequisiteError("Support lifecycle history contains a fork")

    head_ids = [identifier for identifier in by_id if identifier not in referenced]
    if len(head_ids) != 1:
        raise WorkflowPrerequisiteError(
            "Support lifecycle history must contain exactly one head"
        )
    ...
    while True:
        ...
        if current_id is None or current_id in visited:
            raise WorkflowPrerequisiteError(
                "Support lifecycle history contains a cycle"
            )
        ...
        if previous.record.field("to_status") != current.record.field("from_status"):
            raise WorkflowPrerequisiteError(
                "Support lifecycle predecessor status does not reconcile"
            )
        current = previous
    if len(visited) != len(selected):
        raise WorkflowPrerequisiteError(
            "Support lifecycle history contains a disconnected transition"
        )
    return SupportLifecycleState(selected, head)


def require_support_lifecycle_reconciled(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    support: PortiaRecord,
) -> SupportLifecycleState:
    """Require the selected Support lifecycle head to agree with canonical status."""
    state = support_lifecycle_state(repository, work, support)
    selected_status = state.selected_status
    if selected_status is not None and selected_status != support.status:
        raise WorkflowPrerequisiteError(
            "canonical Support status does not reconcile with lifecycle history"
        )
    return state


def require_support_revision_invariants(
    prior: PortiaRecord,
    ...
    require_revision_invariants(
        prior,
        candidate,
        transitions=_SUPPORT_STATUS_TRANSITIONS,
    )
    prior_data = prior.to_dict()
    candidate_data = candidate.to_dict()
    fields = set(prior_data) | set(candidate_data)
    for field in sorted(fields - _SUPPORT_REVISION_MUTABLE_FIELDS):
        if prior_data.get(field) != candidate_data.get(field):
            raise WorkflowPrerequisiteError(
                "ordinary Support lifecycle replacement cannot rewrite "
                f"field {field}"
            )


def require_coordinated_support_transition(
    prior: PortiaRecord,
    candidate: PortiaRecord,
) -> None:
    """Require one legal non-supersession Support lifecycle change."""
    require_support_revision_invariants(prior, candidate)
    if prior.status == candidate.status:
        raise WorkflowPrerequisiteError(
            "Support lifecycle coordination requires a status change"
        )
    if candidate.status == "superseded":
        raise WorkflowPrerequisiteError(
            "Support supersession requires the later correction/adaptation workflow"
        )
    if candidate.status not in {"active", "invalidated"}:
        raise WorkflowPrerequisiteError(
            f"ordinary Support lifecycle cannot select {candidate.status!r}"
        )
    ...
    *,
    ...
    if to_status == "active":
        ...
        if not allow_supersession:
            raise WorkflowPrerequisiteError(
                "Support supersession requires correction/adaptation workflow"
            )
        ...
        raise WorkflowPrerequisiteError(
            f"Support lifecycle cannot select status {to_status!r}"
        )
    if reason_code == "other":
        if not isinstance(reason_detail, str) or not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Support lifecycle reason 'other' requires detail"
            )
    ...
    if reason_detail is not None:
        if not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Support lifecycle reason detail cannot be empty"
            )
        reason["detail"] = reason_detail
    return reason


def build_support_lifecycle_transition(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    prior: PortiaRecord,
    candidate: PortiaRecord,
    *,
    ...
    if allow_supersession:
        ...
        if candidate.status != "superseded":
            raise WorkflowPrerequisiteError(
                "Support successor coordination must supersede its predecessor"
            )
    else:
        require_coordinated_support_transition(prior, candidate)
    state = require_support_lifecycle_reconciled(repository, work, prior)
    ...
    if not isinstance(created_at, str) or not isinstance(created_by, Mapping):
        raise WorkflowPrerequisiteError(
            "Support lifecycle transition requires candidate update provenance"
        )
    effective = effective_at or created_at
    created_time = _parsed_timestamp(created_at, "Support transition created_at")
    effective_time = _parsed_timestamp(effective, "Support transition effective_at")
    support_created = _parsed_timestamp(
        prior.to_dict().get("created_at"),
        "Support created_at",
    )
    if effective_time < support_created:
        raise WorkflowPrerequisiteError(
            "Support lifecycle effective_at cannot precede Support creation"
        )
    if effective_time > created_time:
        raise WorkflowPrerequisiteError(
            "Support lifecycle effective_at cannot follow transition creation"
        )
    ...
    if state.head is not None:
        ...
        if head_id is None:
            raise WorkflowOwnershipError(
                "selected Support lifecycle head has no identity"
            )
