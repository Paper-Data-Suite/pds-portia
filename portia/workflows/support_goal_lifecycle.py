"""Read-side lifecycle reconciliation for canonical ``support_goal@1`` records."""
...
_SUPPORT_GOAL_STATUS_TRANSITIONS = {
...
_SUPPORT_GOAL_REVISION_MUTABLE_FIELDS = frozenset(
    {"status", "updated_at", "updated_by"}
)


@dataclass(frozen=True, slots=True)
class SupportGoalLifecycleState:
    """One exact linear lifecycle chain selected for one Support Goal."""
    ...
    def selected_status(self) -> str | None:
        ...
        if not isinstance(value, str):
            raise WorkflowOwnershipError(
                "selected Support Goal lifecycle head has no valid to_status"
            )
        return value
...
def _target(record: PortiaRecord) -> dict[str, object]:
    if record.logical_id is None:
        raise WorkflowOwnershipError(
            "Support Goal lifecycle target requires an exact Goal identity"
        )
    return {
        "kind": "local_record",
...
def _previous_transition_id(record: PortiaRecord) -> str | None:
    ...
    if not isinstance(previous, Mapping):
        raise WorkflowOwnershipError(
            "Support Goal lifecycle previous_transition is malformed"
        )
    if (
        ...
        raise WorkflowOwnershipError(
            "Support Goal lifecycle previous_transition is not exact"
        )
    return str(previous["record_id"])


def support_goal_lifecycle_state(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    goal: PortiaRecord,
) -> SupportGoalLifecycleState:
    """Resolve persisted Goal lifecycle history without timestamp ordering."""
    _require_owner(work, goal)
    selected = tuple(
        stored
        ...
        if stored.record.field("target") == _target(goal)
    )
    if not selected:
        return SupportGoalLifecycleState((), None)
    ...
    for stored in selected:
        ...
        if transition_id is None or transition_id in by_id:
            raise WorkflowPrerequisiteError(
                "Support Goal lifecycle history repeats a transition identity"
            )
    ...
    if any(identifier not in by_id for identifier in referenced):
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle history references a missing predecessor"
        )
    if len(roots) != 1:
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle history must contain exactly one root"
        )
    if any(count != 1 for count in referenced.values()):
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle history contains a fork"
        )

    heads = [identifier for identifier in by_id if identifier not in referenced]
    if len(heads) != 1:
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle history must contain exactly one head"
        )
    head = by_id[heads[0]]
    visited: set[str] = set()
    current = head
    while True:
        ...
        if current_id is None or current_id in visited:
            raise WorkflowPrerequisiteError(
                "Support Goal lifecycle history contains a cycle"
            )
        ...
        if previous.record.field("to_status") != current.record.field("from_status"):
            raise WorkflowPrerequisiteError(
                "Support Goal lifecycle predecessor status does not reconcile"
            )
        current = previous
    if len(visited) != len(selected):
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle history contains a disconnected transition"
        )
    return SupportGoalLifecycleState(selected, head)


def require_support_goal_lifecycle_reconciled(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    goal: PortiaRecord,
) -> SupportGoalLifecycleState:
    """Require the selected Goal history head to agree with canonical status."""
    state = support_goal_lifecycle_state(repository, work, goal)
    selected = state.selected_status
    if selected is not None and selected != goal.status:
        raise WorkflowPrerequisiteError(
            "canonical Support Goal status does not reconcile with lifecycle history"
        )
    return state



def require_support_goal_revision_invariants(
    prior: PortiaRecord,
    ...
    require_revision_invariants(
        prior,
        candidate,
        transitions=_SUPPORT_GOAL_STATUS_TRANSITIONS,
    )
    prior_data = prior.to_dict()
    candidate_data = candidate.to_dict()
    fields = set(prior_data) | set(candidate_data)
    for field in sorted(fields - _SUPPORT_GOAL_REVISION_MUTABLE_FIELDS):
        if prior_data.get(field) != candidate_data.get(field):
            raise WorkflowPrerequisiteError(
                "ordinary Support Goal lifecycle replacement cannot rewrite "
                f"field {field}"
            )


def require_coordinated_support_goal_transition(
    prior: PortiaRecord,
    candidate: PortiaRecord,
) -> None:
    """Require one legal non-supersession Support Goal lifecycle change."""
    require_support_goal_revision_invariants(prior, candidate)
    if prior.status == candidate.status:
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle coordination requires a status change"
        )
    if candidate.status == "superseded":
        raise WorkflowPrerequisiteError(
            "Support Goal supersession requires the correction workflow"
        )
    if candidate.status not in {"active", "invalidated"}:
        raise WorkflowPrerequisiteError(
            f"ordinary Support Goal lifecycle cannot select {candidate.status!r}"
        )
...
def _transition_reason(
    ...
    *,
    ...
    if to_status == "active":
        ...
        raise WorkflowPrerequisiteError(
            f"Support Goal lifecycle cannot select status {to_status!r}"
        )
    if reason_code == "other":
        if not isinstance(reason_detail, str) or not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Support Goal lifecycle reason 'other' requires detail"
            )
    ...
    if reason_detail is not None:
        if not reason_detail.strip():
            raise WorkflowPrerequisiteError(
                "Support Goal lifecycle reason detail cannot be empty"
            )
        reason["detail"] = reason_detail
    return reason


def build_support_goal_lifecycle_transition(
    repository: PortiaRepository,
    work: ExactPortiaWorkRef,
    prior: PortiaRecord,
    candidate: PortiaRecord,
    *,
    ...
    if allow_supersession:
        ...
        if candidate.status != "superseded":
            raise WorkflowPrerequisiteError(
                "Support Goal successor coordination must supersede its predecessor"
            )
    else:
        require_coordinated_support_goal_transition(prior, candidate)
    state = require_support_goal_lifecycle_reconciled(repository, work, prior)
    ...
    if not isinstance(created_at, str) or not isinstance(created_by, Mapping):
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle transition requires candidate update provenance"
        )
    effective = effective_at or created_at
    created_time = _parsed_timestamp(created_at, "Support Goal transition created_at")
    effective_time = _parsed_timestamp(
        effective,
        "Support Goal transition effective_at",
    )
    goal_created = _parsed_timestamp(
        prior.to_dict().get("created_at"),
        "Support Goal created_at",
    )
    if effective_time < goal_created:
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle effective_at cannot precede Goal creation"
        )
    if effective_time > created_time:
        raise WorkflowPrerequisiteError(
            "Support Goal lifecycle effective_at cannot follow transition creation"
        )
    ...
    if state.head is not None:
        ...
        if head_id is None:
            raise WorkflowOwnershipError(
                "selected Support Goal lifecycle head has no identity"
            )
